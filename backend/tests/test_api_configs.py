...
CONFIGS[2] = CONFIGS[1].replace("set protocols bgp group TRANSIT neighbor 10.0.0.2 peer-as 13335\n", "") + \
...
def fake_collector(monkeypatch):
    ...
    def collect(targets, workers=50, timeout=60):
        return [CollectResult(t.device_id, False, error="connection timed out") if t.hostname in state["fail"]
...
def _device(admin, name="mx204-blr", ip="10.0.0.1"):
    ...
    site_id = site[0]["id"] if site else admin.post("/api/v1/sites", json={"name": "Bangalore", "slug": "blr"}).json()["id"]
    return admin.post("/api/v1/devices", json={"hostname": name, "management_ip": ip, "site_id": site_id,
                                               "platform_id": platforms["junos"], "credential_id": cred["id"]}).json()
...
def test_backup_diff_search_compliance(admin, fake_collector):
    ...
    as_agent(admin, srv["agent_token"]).post("/api/v1/accounting/ingest", json={"lines": [
    ...
    d = admin.get(f"/api/v1/devices/{dev['id']}/diff", params={"old": b1["commit_sha"], "new": b3["commit_sha"],
                                                              "include_inline": True}).json()
...
def test_golden_config_drift(admin, fake_collector):
    dev = _device(admin)
    r = admin.post("/api/v1/golden-configs", json={"name": "ntp", "device_id": dev["id"],
...
def test_change_workflow_and_restore_guard(admin, client, fake_collector, monkeypatch):
    ...
    assert admin.post(f"/api/v1/changes/{cr['id']}/transition", json={"transition": "submit"}).json()["state"] == "pending_approval"
    ...
    assert m.post(f"/api/v1/changes/{cr['id']}/transition", json={"transition": "approve", "comment": "ok"}).json()["state"] == "approved"
    ...
    calls = []
    monkeypatch.setattr(restore_svc, "scrapli_cfg_push",
    ...
    assert admin.post(f"/api/v1/changes/{cr['id']}/transition", json={"transition": "implement"}).json()["state"] == "implemented"
    assert admin.post(f"/api/v1/changes/{cr['id']}/transition", json={"transition": "close"}).json()["state"] == "closed"
