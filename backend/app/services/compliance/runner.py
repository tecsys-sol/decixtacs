...
def run_compliance(db: Session, tenant_id: uuid.UUID, device_ids: list[uuid.UUID] | None = None) -> ComplianceRun:
    ...
    db.add(run)
    db.flush()
    rules_db = list(db.scalars(select(ComplianceRule).where(ComplianceRule.tenant_id == tenant_id, ComplianceRule.enabled)))
    q = select(Device).where(Device.tenant_id == tenant_id).options(
        selectinload(Device.platform), selectinload(Device.site), selectinload(Device.groups))
    if device_ids:
    ...
    for dev in db.scalars(q):
        backup = db.scalar(
            select(ConfigBackup).where(ConfigBackup.device_id == dev.id, ConfigBackup.commit_sha.is_not(None))
        ...
        rules = [Rule(str(r.id), r.name, r.rule_type, r.pattern, r.severity, r.block_start, r.min_count,
                      tuple(r.platforms or ())) for r in applicable]
        ...
        for x in results:
            db.add(ComplianceResult(tenant_id=tenant_id, run_id=run.id, device_id=dev.id, rule_id=uuid.UUID(x.rule_id),
                                    passed=x.passed, detail=x.detail[:2000], backup_id=backup.id))
            if not x.passed:
                metrics.COMPLIANCE_FAILURES.labels(severity=sev[x.rule_id]).inc()
        db.add(DeviceComplianceScore(tenant_id=tenant_id, run_id=run.id, device_id=dev.id, score=score,
                                     passed=len(results) - len(failed), failed=len(failed)))
        scores.append(score)
        crit = [x for x in failed if sev[x.rule_id] in ("high", "critical")]
        if crit:
            emit_event(db, tenant_id, "compliance_failure", severity="high",
                       title=f"{dev.hostname}: {len(crit)} high/critical compliance failure(s)",
                       body="\n".join(x.detail for x in crit[:10]), device_id=dev.id,
