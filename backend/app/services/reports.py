...
def build(
    db: Session, tenant_id: uuid.UUID, report_type: str, period: str = "daily", end: datetime | None = None
) -> Report:
    end = end or utcnow()
    ...
    if report_type in ("device_changes", "config_changes"):
        q = (
            select(
                ...
                ConfigBackup.author,
                ConfigBackup.reason,
                ConfigBackup.lines_added,
                ConfigBackup.lines_removed,
                ConfigBackup.risk_score,
                ConfigBackup.commit_sha,
            )
            .join(Device, Device.id == ConfigBackup.device_id)
            .where(
                ConfigBackup.tenant_id == tenant_id, ConfigBackup.changed, ConfigBackup.collected_at.between(start, end)
            )
        ...
        cmds = dict(
            db.execute(
                ...
                .where(CommandLog.tenant_id == tenant_id, CommandLog.timestamp.between(start, end))
                .group_by(CommandLog.username)
            ).all()
        )
        logins = dict(
            db.execute(
                select(LoginHistory.username, func.count())
                .where(
                    LoginHistory.tenant_id == tenant_id,
                    LoginHistory.success,
                    LoginHistory.timestamp.between(start, end),
                )
        ...
        audits = dict(
            db.execute(
                select(AuditEvent.actor_name, func.count())
                .where(AuditEvent.tenant_id == tenant_id, AuditEvent.timestamp.between(start, end))
        ...
        run = db.scalar(
            ...
            .where(ComplianceRun.tenant_id == tenant_id, ComplianceRun.finished_at.is_not(None))
        ...
        if run:
            q = (
                select(
                    Device.hostname,
                    DeviceComplianceScore.score,
                    DeviceComplianceScore.passed,
                    DeviceComplianceScore.failed,
                )
        ...
        q = (
            select(TacacsAuthEvent.kind, TacacsAuthEvent.result, func.count())
...
def to_pdf(r: Report) -> bytes:
    ...
    story = [
        Paragraph(r.title, styles["Title"]),
        Paragraph(f"{_cell(r.start)} - {_cell(r.end)} UTC - NetworkOps Manager", styles["Normal"]),
        Spacer(1, 12),
        table if r.rows else Paragraph("No data for this period.", styles["Normal"]),
    ]
    doc.build(story)
    return buf.getvalue()
