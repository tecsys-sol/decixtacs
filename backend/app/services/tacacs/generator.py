...
def _junos_block(p: Profile, indent: str) -> list[str]:
    ...
    lines.append(f"{indent}\tset local-user-name = {quote(p.junos_class or ('remote-su' if p.privilege_level >= 15 else 'remote-ro'))}")
...
def render(
    ...
    L.append("# ---------------------------------------------------------------------------")
    L.append("# Generated by NetworkOps Manager - DO NOT EDIT. Changes are overwritten.")
    if header:
        L += [f"# {h}" for h in header.splitlines()]
    L.append("# ---------------------------------------------------------------------------")
    L.append("id = spawnd {")
    L.append(f"\tlisten = {{ port = {s.listen_port} }}")
    L.append(f"\tspawn = {{ instances min = 1 instances max = {s.instances_max} }}")
    L.append("\tbackground = no")
    L.append("}")
    L.append("")
    L.append("id = tac_plus-ng {")
    L.append(f"\tlog accesslog {{ destination = {s.access_log} }}")
    L.append(f"\tlog authzlog {{ destination = {s.authz_log} }}")
    L.append(f"\tlog acctlog {{ destination = {s.acct_log} }}")
    if s.syslog_host:
        L.append(f"\tlog netsyslog {{ destination = {s.syslog_host} }}")
    L.append("\tauthentication log = accesslog")
    L.append("\tauthorization log = authzlog")
    L.append("\taccounting log = acctlog")
    if s.syslog_host:
        L += ["\taccounting log = netsyslog", "\tauthorization log = netsyslog"]
    L.append("")
    ...
    for n in sorted(nas, key=lambda x: ident(x.name)):
        if n.vendor in NO_TACACS_VENDORS:
            ...
            L.append(f"\t# SKIPPED {ident(n.name)} ({n.address}): {n.vendor} does not support TACACS+")
            continue
        L.append(f"\tdevice {ident(n.name)} {{")
        L.append(f"\t\taddress = {n.address}")
        L.append(f"\t\tkey = {quote(n.key)}")
        for t in sorted(set(n.tags)):
            L.append(f"\t\ttag = {ident(t)}")
        L.append("\t}")
    L.append("")
    ...
    for p in profiles:
        if p.timespan:
            L.append(f"\ttimespan ts-{ident(p.name)} {{ {quote(p.timespan)} }}")
    ...
    for u in sorted(users, key=lambda x: x.username):
        L.append(f"\tuser {ident(u.username)} {{")
        if u.auth_method == "crypt":
            if not u.password_crypt:
                warnings.append(f"user {u.username}: no password set - login disabled")
                L.append("\t\tpassword login = deny")
            else:
                L.append(f"\t\tpassword login = crypt {u.password_crypt}")
        else:
            L.append("\t\tpassword login = mavis")
        for g in sorted({ident(g) for g in u.groups}):
            L.append(f"\t\tmember = {g}")
        if u.valid_until:
            L.append(f"\t\tvalid until = {u.valid_until}")
        L.append("\t}")
    L.append("")
    ...
    L.append("\truleset {")
    for p in ordered:
        L.append(f"\t\trule {ident(p.name)} {{")
        L.append("\t\t\tscript {")
        L.append(f"\t\t\t\tif ({_rule_condition(p)}) {{ profile = {ident(p.name)} permit }}")
        L.append("\t\t\t}")
        L.append("\t\t}")
    L.append("\t}")
    L.append("}")
