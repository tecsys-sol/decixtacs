...
export function ImportTacPlusDialog({ open, onOpenChange }: { open: boolean; onOpenChange: (o: boolean) => void }) {
  ...
  const run = useMutation({
    mutationFn: (dryRun: boolean) => api.post<TacacsImportResult>("/tacacs/import", { content, dry_run: dryRun }),
    onSuccess: (r) => {
      if (r.dry_run) {
        setPreview(r);
        return;
      ...
      toast.success("tac_plus configuration imported", "Review the Config preview, then Deploy.");
    },
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[90vh] max-w-4xl overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Classic Shrubbery <code>tac_plus</code> (F4.0.4.x). NAS clients keep their shared keys and users keep their
            password hashes, so devices and engineers need no changes. Nothing is created until you confirm.
          </DialogDescription>
        </DialogHeader>
        ...
        {!done ? (
          <div className="grid gap-2">
            ...
            <p className="text-xs text-muted-foreground">
              The file contains keys and password hashes: it is sent only to this portal over HTTPS and stored
              encrypted.
            </p>
          </div>
        ) : null}
        ...
        {result ? (
          <div className="grid gap-3" data-testid="tacacs-import-result">
            ...
            <div className="grid grid-cols-2 gap-2 sm:grid-cols-5">
              {KINDS.map(([k, label]) => (
                <div key={k} className="rounded-lg border bg-muted/40 p-3">
                  <div className="font-display text-xl font-semibold">{result.created[k]?.length ?? 0}</div>
                  <div className="text-xs text-muted-foreground">{label}</div>
                  {result.skipped[k]?.length ? (
                    <div className="text-[11px] text-muted-foreground">{result.skipped[k].length} already existed</div>
                  ) : null}
                </div>
              ))}
            </div>
            {result.users_needing_password.length ? (
              <div className="rounded-lg border border-warning/40 bg-warning/10 p-3 text-sm" role="status">
                <p className="mb-1 flex items-center gap-2 font-medium text-warning">
                  <KeyRound className="h-4 w-4" /> {result.users_needing_password.length} user(s) without an importable
                  password
                </p>
                <p className="text-xs">
                  {result.users_needing_password.join(", ")} - set a password under TACACS+ → Users (or use LDAP) before
                  the cut-over, otherwise they cannot log in.
                </p>
              </div>
            ) : null}
            {result.warnings.length ? (
              <details className="rounded-lg border border-warning/40 bg-warning/5 p-3" open={result.dry_run}>
                <summary className="flex cursor-pointer items-center gap-2 text-sm font-medium text-warning">
                  <AlertTriangle className="h-4 w-4" /> {result.warnings.length} note(s) to review
                </summary>
            ...
            <details className="rounded-lg border p-3">
              <summary className="cursor-pointer text-sm font-medium">Resulting tac_plus-ng configuration</summary>
              <div className="mt-2">
                <CodeViewer content={result.rendered} filename="tac_plus-ng.cfg" />
              </div>
            </details>
          </div>
        ) : null}
